...
d = d %>% subset(remove_extreme=="") %>%
  mutate(
             response_1 = response_1/truth
           , response_2 = response_2/truth
           , response_3 = response_3/truth
           , raw.truth = truth
           , truth = truth/truth
...
aggreg = myd %>% group_by(network, pair_id, q, party, set) %>% summarize(
               ...
               , prop_improved = mean(err3<err1)
               , prop_worse = mean(err3>err1)
               , prop_same = mean(err3==err1)
...
trial = aggreg %>% group_by(pair_id, set, party, network) %>% summarize(
              ...
              , err_med_1 = mean(err_med_1)
              , err_med_3 = mean(err_med_3)
              , err_med_2 = mean(err_med_2)
              ...
              , prop_improved = mean(prop_improved)
...
save.image("BeckerCentolaPorter_WisdomOfPartisanCrowds_Experiment2.Rdata")
